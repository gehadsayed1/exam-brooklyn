<template>
  ...
  class="w-[200px] border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
>

          <option disabled value="">Choose Instructor</option>
          <option v-for="ins in instructorStore.instructors" :key="ins.id" :value="ins.id">
            {{ ins.name }}
          </option>
        </select>
      </div>
      <div class="flex gap-2">
        ...
        <RouterLink
          ...
          class="bg-gray-200 text-gray-700 px-3 py-2 rounded hover:bg-gray-300 flex items-center gap-1"
        >
    ...
    <div v-if="showForm" class="fixed inset-0 bg-[rgba(0,0,0,0.6)] bg-opacity-30 flex items-center justify-center z-50">
      <div class="bg-blue-100 p-6 rounded-lg shadow-lg w-full max-w-md relative">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input v-model="newInstructor.name" class="w-full border rounded px-3 py-2" type="text" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">Phone</label>
            <input v-model="newInstructor.phone" class="w-full border rounded px-3 py-2" type="text" />
...
const saveInstructor = async () => {
  ...
  if (isEditing.value && editingId.value) {
    await instructorStore.updateInstructor(editingId.value, newInstructor.value)
  } else {

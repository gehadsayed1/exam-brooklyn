<template>
  <div class="w-full overflow-x-auto bg-white rounded-lg shadow-md">
      ...
      <div class="flex flex-col p-2 py-6 m-h-screen">
        <div class="bg-white items-center justify-between w-full flex rounded-full shadow-lg p-2 mb-5 sticky"
          ...
          <input
            ...
            type="text" v-model="search" placeholder="Search by name..." :class="{
    ...
    <!-- Loading Spinner -->
    <div v-if="!filteredItems.length" class="flex justify-center items-center py-20">
      ...
      <table class="min-w-[600px] w-full divide-y text-center divide-gray-200">
        <thead class="bg-gradient-to-r bg-primary text-white">
          <tr>
            ...
            <th v-if="canEdit || canDelete" class="px-6 py-4 text-center text-md font-semibold tracking-wider">
              Actions
            </th>
          </tr>
        </thead>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-gray-50 transition-colors">
            <td v-for="header in headers" :key="header.key" class="px-6 py-4 whitespace-nowrap break-words">
              <!-- Make the 'name' column clickable -->
              <span v-if="header.key === 'name'" @click="showDetails(item)"
            ...
            <td v-if="canEdit || canDelete" class="px-6 py-4 whitespace-nowrap space-x-6">
              ...
              <button v-if="canDelete" @click="$emit('delete', item.id)"
          ...
          <tr v-if="filteredItems.length === 0 && search.length > 0">
            <td :colspan="headers.length + 1" class="px-6 py-4 text-start font-bold text-gray-700">
              No results found for "{{ search }}".
            </td>
          </tr>
          <tr v-if="filteredItems.length === 0 && !loading">
            <td :colspan="headers.length + 1" class="px-6 py-4 text-center font-bold text-gray-600">
              No data found.
            </td>
          </tr>
        </tbody>
...
const canEdit = computed(() => authStore.hasPermission(`edit-${props.resourceType}`));
const canDelete = computed(() => authStore.hasPermission(`delete-${props.resourceType}`));
